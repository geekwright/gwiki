...
<tr><th colspan="4"><{$smarty.const._MD_GWIKI_HISTORY_TITLE}></th></tr>
...
<tr class="<{cycle values="odd,even"}>">
<td><a href="javascript:ajaxGwikiView('<{$revision.keyword}>','<{$revision.gwiki_id}>');" title="<{$smarty.const._MD_GWIKI_HISTORY_VIEW_TT}>" ><{$revision.title}></a></td>

<td><{$revision.revisiontime}><{if $revision.active}>*<{/if}></td>
<td><{$revision.username}></td>
<td><a href="javascript:ajaxGwikiCompare('<{$revision.keyword}>','<{$revision.gwiki_id}>');" title="<{$smarty.const._MD_GWIKI_HISTORY_COMPARE_TT}>" ><{$smarty.const._MD_GWIKI_HISTORY_COMPARE}></a>
<{if $gwiki.mayEdit}> | <a href="javascript:restoreGwikiRevision('<{$revision.keyword}>','<{$revision.gwiki_id}>');" title="<{$smarty.const._MD_GWIKI_HISTORY_RESTORE_TT}>" ><{$smarty.const._AD_GWIKI_RESTORE}></a>
 | <a href="<{$gwiki.modurl}>/edit.php?op=edit&page=<{$revision.keyword}>&id=<{$revision.gwiki_id}>" title="<{$smarty.const._EDIT}>" ><{$smarty.const._EDIT}></a>
<{/if}>
 | <a href="<{$gwiki.modurl}>/showdiff.php?page=<{$revision.keyword}>&id=<{$revision.gwiki_id}>" title="<{$smarty.const._MD_GWIKI_HISTORY_DIFF_TT}>" ><{$smarty.const._MD_GWIKI_HISTORY_DIFF}></a>
</td>
</tr>
<{/foreach}>
</ul>
<{else}>
<tr><td><{$smarty.const._MD_GWIKI_HISTORY_EMPTY}></td></tr>
<{/if}>
</table><br />
<{/if}>
...
window.ajaxGwikiView = function(keyword,id) {
	...
	if((topPos-scrOfY)>250) { document.body.scrollTop=topPos-16; document.documentElement.scrollTop=topPos-16; }
};
window.ajaxGwikiCompare = function(keyword,id) {
	...
	document.body.scrollTop=topPos-16; document.documentElement.scrollTop=topPos-16;
};
<{if $gwiki.mayEdit}>
