...
EOT;
	...
	echo '<tr class="head">'.
		'<th>'._AD_GWIKI_NAMESPACE_PREFIX.'</th>'.
		'<th>'._AD_GWIKI_NAMESPACE_HOME.'</th>'.
		'<th>'._AD_GWIKI_NAMESPACE_AUTONAME_SHORT.'</th>'.
		'<th>'._AD_GWIKI_NAMESPACE_TEMPLATE.'</th>'.
		'<th>'._AD_GWIKI_NAMESPACE_EXTERN_SHORT.'</th>'.
		'<th>'._AD_GWIKI_NAMESPACE_EXTERN_URL.'</th>'.
		'</tr>';
	...
	for ($i = 0; $i < $xoopsDB->getRowsNum($result); $i++) {
		...
		echo '<tr class="'.(($i % 2)?"even":"odd").'"><td><a href="prefixes.php?pid='.$row['prefix_id'].'&op=edit">'.htmlspecialchars($row['prefix'], ENT_QUOTES).'</a></td>' .
			'<td>'.htmlspecialchars($row['prefix_home'], ENT_QUOTES).'</td>'.
			'<td>'.($row['prefix_auto_name']?_YES:_NO).'</td>'.
			'<td>'.$template.'</td>'.
			'<td>'.($row['prefix_is_external']?_YES:_NO).'</td>'.
			'<td>'.htmlspecialchars($row['prefix_external_url'], ENT_QUOTES).'</td>'.
			'</tr>';
	}
	if ($i == 0) {
...
function prefixForm($row,$action) {
		...
		if($action!='new') $form .= '<tr><td class="head">'._AD_GWIKI_NAMESPACE_TEMPLATE.'</td><td class="odd">'.$template.'</td></tr>';
...
function editPrefix($pid) {
	...
	else echo _AD_GWIKI_NAMESPACE_NOT_FOUND;	
...
global $xoopsDB, $wikiPage;
	...
	if(isset($_POST['prefix'])) $row['prefix']=$_POST['prefix'];
	if(isset($_POST['prefix_home'])) $row['prefix_home']=$_POST['prefix_home'];
	
	$row['prefix_auto_name']=0;
	if(isset($_POST['prefix_auto_name']) && $_POST['prefix_auto_name']=='auto') $row['prefix_auto_name']=1;
	
	$row['prefix_is_external']=0;
	if(isset($_POST['prefix_is_external']) && $_POST['prefix_is_external']=='external') $row['prefix_is_external']=1;
	
	if(isset($_POST['prefix_external_url'])) $row['prefix_external_url']=$_POST['prefix_external_url'];
	...
	else $message=_MD_GWIKI_ERRORINSERT;
...
function newTemplate($pid) {
	...
	$row['template_body']=file_get_contents('../templates/gwiki_view.html');
...
function tobedone($op,$pid) {
	...
	$pid=0;$op='';
	// get variables
	if(!empty($_GET['pid'])) $pid=intval($_GET['pid']);
	if(!empty($_GET['op'])) $op=cleaner($_GET['op']);
	// override get with post
	if(!empty($_POST['pid'])) $pid=intval($_POST['pid']);
	if(!empty($_POST['op'])) $op=cleaner($_POST['op']);
